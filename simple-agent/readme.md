...
> 从零搭建、可扩展的中文智能 Agent。具备工具调用、三层记忆、流式 Web 界面、错误重试、并发执行、完整单测。本文档逐文件、逐功能详细介绍。
...
4. [核心能力与文件详解](#4-核心能力与文件详解)
5. [配置项总览](#5-配置项总览)
6. [扩展指南](#6-扩展指南)
7. [Skill 系统（按需加载的能力包）](#7-skill-系统按需加载的能力包)
8. [部署到生产](#8-部署到生产)
9. [常见问题与平台专项](#9-常见问题与平台专项)
10. [附录](#10-附录)
...
**对话能力**：LLM 推理 → 工具调用 → 工具结果 → 再推理，多轮自动循环；一轮内多个工具可并行执行；工具异常 / LLM 失败均不会让进程崩溃，会以错误信息回灌给 LLM 自我纠错。

**记忆能力**（三层独立可开关）：
- **短期**：进程内对话窗口 + 自动摘要压缩
- **会话持久化**：SQLite 按 `session_id` 存取，进程重启可恢复
- **长期向量**：Chroma 语义检索，跨会话"记得用户偏好"

**界面能力**：命令行（`python agent.py`） + 浏览器 Web UI（FastAPI + SSE 流式）。

---

## 2. 5 分钟快速开始

> 假设是一台**全新的 macOS / Linux / Windows 机器**。从克隆代码到看到 Agent 回复，7 步搞定。

### 2.1 前置准备（一次性）

| 必需 | 检查命令 | 没有的话 |
|---|---|---|
| Python ≥ 3.10 | `python3 --version`（Win 用 `python --version`） | macOS：`brew install python@3.13`<br>Linux：`apt install python3 python3-venv`<br>Windows：[python.org](https://www.python.org/downloads/windows/) 下载，**勾选 ✅ Add Python to PATH** |
| pip | `python -m pip --version` | 一般随 Python 自带；缺则 `python -m ensurepip` |
| LLM API Key | —— | 推荐 [DeepSeek 控制台](https://platform.deepseek.com/)，注册后充 1 元即可 |

> Windows 用户优先使用 **PowerShell**（Win+X 选"终端"），不要用老 cmd.exe。
...
> 若 PowerShell 报 `无法加载文件 ... 因为在此系统上禁止运行脚本`，**管理员**身份打开 PowerShell 执行一次：
> ```powershell
...
> 想用 OpenAI / 通义千问？参考 [§6.2 切换 LLM 服务商](#62-切换-llm-服务商)。
...
预期输出 `Ran 39 tests in 0.17s / OK`。**39 个用例完全离线**，不需要真实 API Key 也能通过——这步通过则说明依赖和代码 OK，后续问题只可能在 LLM key。
...
- 左侧 `+ 新会话`，右侧输入框回车发送（Shift+Enter 换行）
- 工具调用以气泡形式实时流式渲染
...
重启服务后，CLI/Web 历史依然在，且 Agent 会自动 `recall` 相关历史片段拼到 prompt，实现"长期记忆"。

### 出问题怎么办？

1. **先看终端日志**：错误一般直接打印在 `uvicorn` 或 `python agent.py` 的输出里。
2. **再看 [§9 FAQ](#9-常见问题与平台专项)**：覆盖了 10 个最常见坑。
3. **重置大法**：删掉 `.venv` / `sessions.db` / `chroma_store`，从 §2.3 重做一遍，95% 问题能解决。
...
- `chat_with_retry(messages, tools=None, retries=None)` —— 生产入口：
...
**为什么这样设计**：LLM 抖动是常态，但**不能无限重试**——5xx 多半是模型过载几秒就好；4xx 是参数错重试无意义。加 jitter 防止多 worker 同步重试雪崩。
...
**为什么这样设计**：工具抛异常 → 主循环崩 → 对话中断。错误应该被翻译成"文本"回灌给 LLM，让它自己决定"换工具"或"问用户"。区分 `TypeError`（参数不匹配，多半是 LLM 格式错误）与其他异常便于调试；返回值强制转 `str`，满足 OpenAI `tool` 消息的 `content` 字段要求。
...
**关键方法**：`append / load / list_sessions / delete`。
...
零构建、纯原生（HTML/CSS/JS）实现 ChatGPT 风格界面。
...
**响应式断点**：`>1024px` 侧栏 260px / `≤1024px` 侧栏 220px / `≤768px` 抽屉。
...
| 文件 | 说明 |
|---|---|
| [`.env.example`](.env.example) | 配置模板（入库），含所有可调项 |
| `.env` | 实际配置（`.gitignore`，不入库）。最小可用：`LLM_API_KEY` + `LLM_BASE_URL` + `LLM_MODEL` |
| `sessions.db` | SQLite 数据文件。启用 `ENABLE_SESSION_STORE=1` 后由 [`SessionStore`](memory/session_store.py) 自动创建。可用 `sqlite3` CLI / DB Browser 查看历史 |
| `chroma_store/` | Chroma 向量库目录。启用 `ENABLE_LONG_TERM=1` 后由 [`long_term`](memory/long_term.py) 自动创建。**删除目录即清空所有长期记忆** |
| `.venv/` | Python 虚拟环境，含独立 python/pip/site-packages，与系统 Python 完全隔离。**不要跨机器复制**，应通过 `pip freeze > requirements.txt` 重建 |

---

## 5. 配置项总览

所有配置通过 `.env` / 环境变量注入，代码里均有默认值——零配置也能跑起来（除 `LLM_API_KEY` 必填）。
...
重启服务即可，LLM 会自动感知新工具。建议同步在 `prompts.py` 的"可用工具"列表里加一行，并写一个 `test_` 单测。
...
借鉴 Claude / JoyCode 的 Skill 设计：把"领域知识 + 操作指南 + 辅助资源"打包成一个 skill 目录，Agent 在合适时机自动激活。
...
### 8.3 方案 A：裸机 + systemd + Nginx + HTTPS

#### Step A1：上传代码
```bash
cd ~
git clone https://your-git-repo/simple-agent.git
# 或本地 scp 上传： scp -r simple-agent deploy@your.ip:~/
cd simple-agent
```
...
certbot 会自动：申请 Let's Encrypt 证书 → 改写 nginx 配置加 `listen 443 ssl` → 配置 80 → 443 强制跳转 → 安装 cron 自动续签（每 90 天）。完成后浏览器打开 `https://agent.yourdomain.com` 即可。
...
### 8.4 方案 B：Docker / docker-compose

适合「不想折腾 systemd 和 Python 环境」的同学。

#### Step B1：装 Docker

不同操作系统下安装方式差异较大，按你的开发/部署机选对应方案即可。**生产服务器一般是 Linux**，本地开发可能是 macOS / Windows。

**🐧 Linux（Ubuntu / Debian / CentOS）** —— 官方一键脚本：
```bash
...
1. 访问 [docker.com/products/docker-desktop](https://www.docker.com/products/docker-desktop/) 下载对应芯片的安装包（Apple Silicon 选 *Mac with Apple chip*，Intel 选 *Mac with Intel chip*）
2. 双击 `.dmg`，把 Docker.app 拖进 Applications
3. 启动 Docker Desktop，等顶部状态栏小鲸鱼图标变绿

> **公司电脑** Docker Desktop 有商业 License 限制，可用开源替代 **Colima**：
> ```bash
> brew install colima docker docker-compose docker-buildx
> colima start --cpu 4 --memory 8 --disk 60
> ```
> 国内网络若 `docker run hello-world` 卡在 DNS / 拉镜像超时：
> ```bash
> # ① 换 DNS
> colima stop && colima start --dns 8.8.8.8 --dns 223.5.5.5
> # ② 配镜像加速器（拷到 VM 的 /etc/docker/daemon.json 后重启 docker）
> ```

**🪟 Windows** —— 推荐 **Docker Desktop for Windows**（基于 WSL2）：
1. 启用 WSL2（PowerShell 管理员）：
   ```powershell
   wsl --install
   # 重启电脑
   wsl --set-default-version 2
   ```
2. 下载安装 Docker Desktop for Windows，安装时勾选 ✅ **Use WSL 2 instead of Hyper-V**
3. Settings → Resources → WSL Integration，开启 Ubuntu 集成

> **强烈建议** Windows 用户在 **WSL Ubuntu** 终端里跑后续命令（PowerShell 直接 build chromadb 时 C 扩展编译易失败）。进入 WSL 后用 `/mnt/d/path/to/simple-agent` 访问 Windows 盘符。
...
**Q8：同时跑 CLI 和 Web 会互相覆盖 session 吗？**
...
Windows 默认终端编码是 GBK，遇到中文可能乱码。永久解决（可写进 PowerShell Profile `$PROFILE`）：
```powershell
...
代码里所有文件路径都用了 `os.path.join` 或 `pathlib`，Windows 上自动变成 `\`，**无需改代码**。只有写 `.env` 时需要注意：
```bash
# ✅ 正确（正斜杠，跨平台）
